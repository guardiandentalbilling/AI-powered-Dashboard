...
// CORS configuration (dev relaxed to aid local / file:// testing)
const devAllowed = ['http://localhost:3000','http://localhost:5000','http://127.0.0.1:5000'];
const prodAllowed = ['https://yourdomain.com','https://www.yourdomain.com']; // TODO: replace with real domains
const corsOptions = {
    origin: function(origin, callback){
        // Allow same-origin / server-to-server (no origin header) and development relaxed usage
        if(!origin){ return callback(null,true); }
        if(process.env.NODE_ENV !== 'production'){
            // Allow file:// (appears as "null") and any localhost variants for easier testing
